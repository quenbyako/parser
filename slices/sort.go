...
func Sort[S ~[]E, E constraints.Ordered](s S) S {
	n := len(s)
	pdqsortOrdered(s, 0, n, bits.Len(uint(n)))

	return s
}

func SortEq[S ~[]T, T constraints.Compare[T]](x S) S {
	return SortFunc(x, func(a, b T) bool { return a.Cmp(b) < 0 })
}
...
func SortStableFunc[E any](x []E, less func(a, b E) bool) []E {
...
func nextPowerOfTwo(length int) uint { return 1 << bits.Len(uint(length)) }
